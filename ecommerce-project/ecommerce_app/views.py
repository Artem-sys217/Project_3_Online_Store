...
def index(request):
    ...
    pets_products = Product.objects.filter(category__name="Товары для животных")
    tv_audio_video = Product.objects.filter(category__name="ТВ, Аудио, Видео")
    jewelry = Product.objects.filter(category__name="Украшения")
    beauty_and_health = Product.objects.filter(category__name="Красота и здоровье")
    home_appliances = Product.objects.filter(category__name="Бытовая техника")
    computers_and_peripherals = Product.objects.filter(category__name="Компьютеры и периферия")
    clothes_and_shoes = Product.objects.filter(category__name="Одежда и обувь")
    phones_and_gadgets = Product.objects.filter(category__name="Телефоны и гаджеты")
    ...
    return render(request, "ecommerce_app/index.html", {
        ...
        'phones_and_gadgets': phones_and_gadgets,
    })
